...
window.addEventListener('load', function () {
    ...
    function drawStonks() {
        ctx1.fillStyle = 'rgb(70, 0, 100)'; // Text color
        ctx1.font = 'bold 36px "Libre Baskerville", serif';
        ctx1.textAlign = 'center'; // Center the text
        ctx1.textBaseline = 'middle'; // Align text vertically
        let stonks = document.getElementById('stonks');
        ctx1.drawImage(stonks, 0, 0, stonks.width, stonks.height, ctx1.canvas.width * 0.03, 
        ctx1.canvas.height * 0.4, stonks.width * 0.4, stonks.height * 0.4);
    }
    ...
    function drawRefresh() {
        ...
        const imgX = ctx1.canvas.width * 0.83; // Calculate X position
        const imgY = ctx1.canvas.height * 0.43; // Calculate Y position
        const imgWidth = refresh.width * 0.5; // Calculate Image Width
        const imgHeight = refresh.height * 0.5; // Calculate Image Height
    
        // Draw the image on the canvas
        ctx1.drawImage(refresh, 0, 0, refresh.width, refresh.height, imgX, imgY, imgWidth, imgHeight);
    }
