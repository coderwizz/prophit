const visualPairs = [ // for Visual only 
    [380, '$1 per calorie in this burger'],
    [1010, '$1 per calorie in this burger'],
    [942, '$1 per calorie in this pizza'],
    [929, '$1 for every 5 pounds this car weighs'],
    [178, 'Imagine each cell red/white, equally likely. $100(#trees on avg if at least 1)'],
    [450, '$1 per calorie in this drink'],
    [357, 'One cent for every dollar this artifact is worth today'],
    [258, '$2 for each episode from this show'],
    [510, '$2 for each episode from this show'],
    [886, '$1 for every jelly bean in this jar'],
    [605, '$5 for every distinct object in this image (background doesn\'t count)'],
];
...
export class Balloon extends Challenge {
    ...
    displayQuestion() {
        ...
        // actual
        context.fillStyle = 'rgb(57, 86, 171)'; // Text color
        context.font = 'bold 19px "Libre Baskerville", serif';
        context.textAlign = 'center'; // Center the text
        context.textBaseline = 'middle'; // Align text vertically
...
export class Exponential extends Challenge {
    constructor() {
        ...
        // Only set the random question once if it has not already been set
        if (!this.lambda) {
            this.lambda = (Math.floor(Math.random() * 9) * 2 + 21) / 100;
        }
        this.reviewed = 0;
        if (this.reviewed === 0) {
...
export class Visual extends Challenge {
    constructor() {
        ...
        if (!this.frameX && !this.frameY) {
            let idx = Math.floor(Math.random() * 11);
            this.frameY = Math.floor(idx / 4);
            this.frameX = idx % 4;
        }
        this.image = document.getElementById('visual');
        this.value = visualPairs[this.frameY * 4 + this.frameX][0];
    }
    displayQuestion() {
        const canvas = document.getElementById('canvas2');
        const context = canvas.getContext('2d');
        context.clearRect(0, 0, context.canvas.width, context.canvas.height); // Clear the canvas
        // Define the margin values you want to trim from the top and right
        const marginTop = 10; // Example value for the top margin
        const marginRight = 10; // Example value for the right margin

        context.drawImage(
            this.image, 
            this.frameX * this.image.width / 4, // sx: Source X (horizontal position of the frame in the image)
            this.frameY * this.image.height / 3 + marginTop, // sy: Adjust for the top margin
            this.image.width / 4 - marginRight, // sWidth: Width of each frame minus right margin
            this.image.height / 3 - marginTop, // sHeight: Height of each frame minus top margin
            context.canvas.width * 0.37, // dx: Destination X (where on the canvas to start drawing)
            context.canvas.height * 0.27, // dy: Destination Y (where on the canvas to start drawing)
            1.6 * (this.image.width / 4 - marginRight), // dWidth: Width of the drawn image minus right margin
            1.6 * (this.image.height / 3 - marginTop) // dHeight: Height of the drawn image minus top margin
        );
        // guide 
        context.fillStyle = 'rgb(116, 7, 130)'; // Text color
        context.font = 'bold 42px "Libre Baskerville", serif';
        ...
        context.textAlign = 'center'; // Center the text
        context.textBaseline = 'middle'; // Align text vertically
        context.fillText(visualPairs[this.frameY * 4 + this.frameX][1], canvas.width / 2, canvas.height * 0.75); // Draw the question text
    }    
    ...
    displayValue(game) {
        ...
        context.textAlign = 'center'; // Center the text
        context.textBaseline = 'middle'; // Align text vertically
        context.fillText(visualPairs[this.frameY * 4 + this.frameX][1], canvas.width / 2, canvas.height * 0.8); 
        // actual
        context.fillStyle = 'rgb(55, 180, 86)'; // Text color
        context.font = 'bold 120px "Libre Baskerville", serif';
        context.textAlign = 'center'; // Center the text
